...
public sealed partial class SurgeryBui : BoundUserInterface
{
    ...
    private void Update(SurgeryBuiState state)
    {
        ...
        foreach (var choice in state.Choices.Keys)
            ...
            {
                if (_entities.TryGetComponent(ent, out OrganComponent? organ))
                    options.Add((choice, ent.Value, _entities.GetComponent<MetaDataComponent>(ent.Value).EntityName, organ.Category));
        ...
        options.Sort((a, b) =>
        {
            int GetScore(ProtoId<OrganCategoryPrototype>? category)
            {
                return category?.Id switch
                {
                    "Head" => 1,
                    "Torso" => 2,
                    "ArmLeft" or "ArmRight" => 4,
                    "HandLeft" or "HandRight" => 5,
                    "LegLeft" or "LegRight" => 6,
                    "FootLeft" or "FootRight" => 7,
                    null => 10,
                    _ => 9
                };
            }
    ...
    private void AddStep(EntProtoId stepId, NetEntity netPart, EntProtoId surgeryId)
    {
        ...
        var stepButton = new SurgeryStepButton { Step = step, SurgeryId = surgeryId, StepId = stepId };
    ...
    private void RefreshUI()
    {
        if (_window == null
            ...
            || _player.LocalEntity is not { } localPlayer)
            return;

        var next = _system.GetNextStep(Owner, _part.Value, _surgery.Value.Ent, localPlayer);
        var i = 0;
