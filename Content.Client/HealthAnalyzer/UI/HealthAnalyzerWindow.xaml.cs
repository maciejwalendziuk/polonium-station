...
public sealed partial class HealthAnalyzerWindow : FancyWindow
{
    ...
    /// <summary>
    /// Whether a dead patient could be shocked back, and if not why not. Sits at the top of the
    /// Conditions list on every tab; nothing is shown for the living (readiness None).
    /// </summary>
    private void AddDefibrillationCondition(DefibrillationReadiness readiness)
    {
        var key = readiness switch
        {
            DefibrillationReadiness.Hopeless => "health-analyzer-window-cannot-defibrillate-hopeless-text",
            DefibrillationReadiness.TooMuchDamage => "health-analyzer-window-cannot-defibrillate-damage-text",
            DefibrillationReadiness.NeedsAdrenaline => "health-analyzer-window-cannot-defibrillate-adrenaline-text",
            DefibrillationReadiness.Risky => "health-analyzer-window-can-defibrillate-risky-text",
            DefibrillationReadiness.Ready => "health-analyzer-window-can-defibrillate-text",
            _ => null,
        };
    ...
    public void Populate(HealthAnalyzerBodyMessage msg)
    {
        ...
        AddDefibrillationCondition(msg.Defibrillation);
        ...
        foreach (var (bodyPart, isBlocked) in msg.BlockedHealing)
        {
            if (!isBlocked || isPart && bodyPart != selectedBodyPart)
                continue;

            ConditionsListContainer.AddChild(new RichTextLabel
            {
                Text = Loc.GetString($"condition-body-blocked-healing-{bodyPart}", ("entity", Identity.Name(_target.Value, _entityManager))),
                Margin = new Thickness(0, 4),
            });
        }
    ...
    public void Populate(HealthAnalyzerOrgansMessage msg)
    {
        ...
        foreach (var (organ, data) in msg.Organs)
        {
            ...
            if (data.Severity == OrganSeverity.Normal)
                continue;
